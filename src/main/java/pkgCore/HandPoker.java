...
public class HandPoker extends Hand implements Comparable {
	 ...
	 * @since Lab #2
	 * 
	 * EvaluateHand - evaluate the hand calling methods that use reflections
	 * @param hp
	 * @return
	 ...
	 * @since Lab #2
	 * 
	 * ScoreHand - this method will score the hand.  It will dynamically call the frequency method to 
	...
	private boolean isStraightFlush() {
		...
		// TODO - Complete implementation for this method. DONE
	...
	private boolean isFlush() {
		...
		for (iPos = 0; iPos < super.getCards().size(); iPos++) {
			if (super.getCards().get(iPos).geteSuit() == eSuit.CLUBS) {
				clubsCount++;
			}
			if (super.getCards().get(iPos).geteSuit() == eSuit.HEARTS) {
				heartsCount++;
			}
			if (super.getCards().get(iPos).geteSuit() == eSuit.DIAMONDS) {
				diamondsCount++;
			}
			if (super.getCards().get(iPos).geteSuit() == eSuit.SPADES) {
				spadesCount++;
			}
		}
		if (clubsCount == 5 || heartsCount == 5 || diamondsCount == 5 || spadesCount == 5) {
			bisFlush = true;
		}
		else {
			bisFlush = false;
	...
	private boolean isStraight() {
		...
		int iPos;
		int count = 0;
		if (GetCRCSize() == eRowCount.FIVE.getiRowCountItems()) {
			// if top card rank - low card rank = 4 -> true
		}
